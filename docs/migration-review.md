...
5. 审查发现附件若保存在公开上传目录可能形成同源存储型 XSS；新附件已迁入 `WEB-INF/private-uploads`，旧公开地址强制转到鉴权下载接口。
...
- 最终验证结果：清理旧页面专用测试并补充兼容回归测试后，后端 68 个测试、前端 ESLint、13 个 Vitest、TypeScript 严格检查、生产构建和 WAR 打包全部通过。
