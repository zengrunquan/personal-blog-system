...
- 分页查询（自定义PageUtil工具类）
- 多表关联查询
- 首页分类统计（HomeServlet加载分类及真实文章数量，JSP服务端渲染）
- 事务管理（分类删除级联清理评论）
- XSS防护（输入转义 + 输出转义）
- CSRF防护（Token验证）
- 密码安全（新密码使用 BCrypt 随机盐哈希，旧 MD5 账号登录成功后自动升级）
- 统一数据库错误日志（DAO 和 DBUtil 使用 Log4j2 记录操作上下文和异常堆栈）
- Service 构造器依赖注入（保留默认 DAO 装配，并支持通过接口注入可替换依赖）
- 响应式设计（适配桌面/平板/手机）
...
21. **后台用户列表缺少删除入口** - 用户管理页面已补充删除按钮，并通过 Ajax 调用后台删除接口
22. **密码使用无盐MD5哈希** - 新密码已改用成本因子 12 的 BCrypt，旧 MD5 账号登录成功后自动升级
23. **数据库异常缺少统一上下文** - DAO 和 DBUtil 已改用 Log4j2 记录操作上下文和异常堆栈
24. **Service直接依赖具体DAO实现** - 四个 ServiceImpl 已支持 DAO 接口构造器注入，并保留兼容现有 Servlet 的无参构造器
25. **首页分类文章数量固定为0** - 新增 `HomeServlet` 调用分类关联统计查询，首页通过JSTL渲染数据库中的分类名称和文章数量，不再使用JavaScript硬编码分类及数量
