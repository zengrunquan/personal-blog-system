...
public class UserDaoImpl implements UserDao {
    ...
    public boolean insert(User user) {
        String sql = "INSERT INTO user (username, password, nickname, email, avatar, bio, role, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DBUtil.getConnection();
            ...
            ps.setInt(7, user.getRole() != null ? user.getRole() : 0);
            ps.setInt(8, user.getStatus() != null ? user.getStatus() : 1);
    ...
    public boolean update(User user) {
        ...
        String sql = "UPDATE user SET nickname = ?, email = ?, bio = ? WHERE id = ?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
